...
HLEDGER_LINK="$HOME/.hledger.journal"
HLEDGER_REPO_DIR="$HOME/GitHub/INTxK/Accounting-Management-System"
HLEDGER_REPO_DATA_DIR="$HLEDGER_REPO_DIR/hledger/data"
HLEDGER_REPO_FILE="$HLEDGER_REPO_DATA_DIR/hledger.journal"
HLEDGER_REPO_HIDDEN_FILE="$HLEDGER_REPO_DATA_DIR/.hledger.journal"
...
if [[ -d "$HLEDGER_REPO_DIR" ]]; then
    ...
    if [[ -L "$HLEDGER_LINK" ]]; then
        ...
        current_target=$(readlink "$HLEDGER_LINK")
