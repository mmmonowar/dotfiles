...
| `git-health.sh` | Git Health | `gh-check-all` (pre-flight checks for stale MERGE_HEAD, lock files, dirty worktree, bogus filenames, CRLF, divergent branches, config audit), `gh-report` (diagnostics summary). |
| `config-audit.sh` | Config Audit & Merge | `ca_check_symlinks` (audit managed configs), `ca_offer_merge` (interactive per-hunk merge for diverged files), `ca_repair_symlinks` (fix broken symlinks). |
