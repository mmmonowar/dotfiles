...
# Only auto-launch when POLYTERM_MULTIPLEXER is explicitly set to "tmux" or "zellij"
if [[ -o interactive ]] && [[ -z "$TMUX" ]] && [[ -z "$ZELLIJ" ]] && [[ -n "$POLYTERM_MULTIPLEXER" ]]; then
    if [[ "$POLYTERM_MULTIPLEXER" == "zellij" ]] && command -v zellij &>/dev/null; then
        (
...
# 6. AI & LLM ALIASES
# -----------------------------------------------------------------------------
if [[ "$(git config user.email 2>/dev/null)" == "${POLYTERM_USER_EMAIL:-}" ]] && [[ -n "$POLYTERM_USER_EMAIL" ]]; then
...
# 7. LOCAL OVERRIDES (Not synced via Git)
# -----------------------------------------------------------------------------
